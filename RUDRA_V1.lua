-- RUDRA V1 | UNITED
-- FINAL CLIENT SCRIPT (BROOKHAVEN SAFE)
-- DELTA EXECUTOR COMPATIBLE

-- ================= REMOTE KILL SWITCH =================
local STATUS_URL = "https://raw.githubusercontent.com/YOUR_USERNAME/YOUR_REPO/main/status.txt"

local success, response = pcall(function()
	return game:HttpGet(STATUS_URL)
end)

if not success or not response or not string.find(response, "ON") then
	warn("RUDRA V1 | SCRIPT DISABLED BY DEVELOPER")
	return
end
...
targetBox.Position = UDim2.new(0.1,0,0.25,0)
...
targetBox.FocusLost:Connect(function()
	targetName = string.gsub(targetBox.Text, "@", "")
end)

-- DELAY BOX
local delayBox = Instance.new("TextBox", frame)
delayBox.Size = UDim2.new(0.4,0,0,26)
delayBox.Position = UDim2.new(0.3,0,0.4,0)
...
startBtn.Size = UDim2.new(0.4,0,0,26)
...
stopBtn.Size = UDim2.new(0.4,0,0,26)
...
task.spawn(function()
	while true do
		if active then
			local t = targetName ~= "" and (" "..targetName) or ""
			send(string.rep("_", LINE_LEN)..t.." "..SPAM_TEXT[index])
			index = index % #SPAM_TEXT + 1
			task.wait(spamDelay)
		else
			task.wait(0.2)
		end
...
local function startSpam()
	...
	send(string.rep("_", START_WARNING_LEN) ..
		" Spam starting in 20 second | SCRIPT BY RUDRA")
...
print("RUDRA V1 | UNITED | FINAL & LOADED")
