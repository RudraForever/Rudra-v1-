...
-- ================= SPAM WARNING =================
local WARNING_LEN = 140
local WARNING_TEXT = " spam starting in 20 seconds | Made by Rudra"
...
if not allowed then
	warn("ACCESS DENIED | NOT REGISTERED")
	return
end
...
frame.Position = UDim2.new(0.5,-180,0.45,-165)
...
-- PATTERN BOX
local patternBox = Instance.new("TextBox", frame)
patternBox.Size = UDim2.new(0.8,0,0,26)
patternBox.Position = UDim2.new(0.1,0,0.44,0)
patternBox.PlaceholderText = "Pattern (____ / -_- / @_@)"
patternBox.BackgroundColor3 = Color3.fromRGB(25,25,25)
patternBox.TextColor3 = Color3.fromRGB(255,255,255)
patternBox.BorderSizePixel = 0
...
task.spawn(function()
	while true do
		if active then
			local prefix = patternBox.Text ~= "" and (patternBox.Text .. " ") or ""
			local target = targetName ~= "" and (" " .. targetName) or ""

			send(prefix .. TMKX_LIST[tIndex] .. target)

			tIndex += 1
			if tIndex > #TMKX_LIST then
				tIndex = 1
			end
		end
		task.wait(spamDelay)
...
local function startSpam()
	...
	send(string.rep("_", WARNING_LEN) .. WARNING_TEXT)
...
warn("RUDRA V1 | LOADED | ACCESS GRANTED")
